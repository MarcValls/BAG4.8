...
export function ControlPlane() {
  ...
  const runContextCommand = async (command: string) => {
    ...
    if (result?.data && command.includes('inspect')) {
      onInspect({
        id: 'context-inspect',
        kind: 'context',
        title: 'Inspección de contexto',
        summary: result.message || 'Contexto inspeccionado',
  ...
  const paletteActions = useMemo(() => {
    const base: Array<{ id: string; label: string; action: () => void }> = [
      ...
      { id: 'ctx-inspect', label: 'Inspeccionar contexto', action: () => void runContextCommand('/context inspect') },
      ...
      {openingLayer || (
        <div className={`app-root mode-${uiState.globalMode} ${uiState.sidebarCollapsed ? 'sidebar-collapsed' : ''} ${selection ? 'has-inspector' : ''}`}>
          ...
          <div className="app-body">
            ...
            <WorkspaceShell activeSection={uiState.activeSection} snapshot={snapshot} mode={uiState.globalMode}>
              <ControlSections
