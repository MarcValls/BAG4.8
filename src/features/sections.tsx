...
function roadmapIterations(roadmap: unknown): RecordValue[] {
  const data = asRecord(roadmap);
  return data && Array.isArray(data.iterations) ? data.iterations as RecordValue[] : [];
}
...
export function ControlSections(props: Props) {
  ...
  const [workspaceFilter, setWorkspaceFilter] = useState<'all' | 'file' | 'directory'>('all');
  const [workspaceLayout, setWorkspaceLayout] = useState<'list' | 'board'>('list');
  ...
  const visibleFiles = useMemo(() => {
    ...
    return allFiles.filter((entry) => {
      ...
      const matchesType = workspaceFilter === 'all'
        || (workspaceFilter === 'directory' ? ['directory', 'dir', 'folder'].includes(type) : !['directory', 'dir', 'folder'].includes(type));
  ...
  const roadmapResult = props.commandResults.roadmap;
  const plan = planResult?.data || planResult?.plan || planResult;
  const roadmap = roadmapResult?.data || roadmapResult;
  const steps = planSteps(plan);
  const iterations = roadmapIterations(roadmap);
  const planData = asRecord(plan);
  const pipelineStatus = String(planData?.status || (planResult?.ok === false ? 'failed' : steps.length ? 'running' : 'pending'));
    ...
    return (
      <div className="home-surface">
        <section className="home-hero">
          <div>
            <StatusBadge status={snapshot?.system.state || 'unknown'} label={snapshot?.system.backendAvailable ? 'Backend conectado' : 'Backend no disponible'} />
            <h2>{snapshot?.system.objective || '¿Qué quieres resolver ahora?'}</h2>
            <p>El chat coordina la tarea. Workspace y Revisión amplían la misma sesión sin abrir una interfaz paralela.</p>
          </div>
        ...
        <section className="home-destinations" aria-label="Superficies principales">
          <button type="button" className="destination-card primary-destination" onClick={() => props.onSetSection('chat')} disabled={!canChat}>
            <span className="destination-icon"><Icon name="chat" size={25} /></span>
            <span className="destination-copy">
              <strong>Chat</strong>
              <small>Conversar, decidir y ejecutar acciones.</small>
            </span>
            <Icon name="arrowRight" />
          </button>
          <button type="button" className="destination-card" onClick={() => props.onSetSection('workspace')}>
            <span className="destination-icon"><Icon name="workspace" size={25} /></span>
            <span className="destination-copy">
              <strong>Workspace</strong>
              <small>Explorar proyecto, sesiones y artefactos.</small>
            </span>
            <Icon name="arrowRight" />
          </button>
          <button type="button" className="destination-card" onClick={() => props.onSetGlobalMode('review')}>
            <span className="destination-icon"><Icon name="review" size={25} /></span>
            <span className="destination-copy">
              <strong>Revisión</strong>
              <small>Leer resultados y evidencias sin ruido.</small>
            </span>
            <Icon name="arrowRight" />
          </button>
        </section>
        ...
        <section className="home-status-panel">
          <div className="panel-heading">
            <div>
              <span className="surface-eyebrow">Estado resumido</span>
              <h3>Sesión operativa</h3>
            </div>
            <button className="text-button" type="button" onClick={() => props.onSetSection('system')}>Ver sistema <Icon name="chevron" size={14} /></button>
          </div>
          <div className="metrics-row">
            <Metric label="Workspace" value={snapshot?.workspace.id || 'No confirmado'} hint={snapshot?.workspace.linkedToSession ? 'Vinculado a sesión' : 'Pendiente de vínculo'} icon="workspace" />
            <Metric label="Modelo" value={snapshot?.model.effectiveModel || snapshot?.model.configuredModel || 'Desconocido'} hint={snapshot?.model.provider || 'Provider desconocido'} icon="model" />
            <Metric label="Contexto" value={snapshot?.context.state || 'unknown'} hint={snapshot?.context.revision ? `Revisión ${snapshot.context.revision}` : 'Sin receipt'} icon="context" />
            <Metric label="Sesión" value={snapshot?.session.id || 'No iniciada'} hint={snapshot?.session.state || 'unknown'} icon="session" />
          </div>
        </section>
      </div>
    );
      ...
      <div className={`chat-surface ${props.globalMode === 'focus' ? 'is-focus' : ''}`}>
        <div className="chat-toolbar">
          <div className="segmented-control" aria-label="Modo de chat">
            ...
            <button type="button" onClick={() => props.onSetGlobalMode('focus')}>
              <Icon name="focus" size={15} /> Focus
            </button>
          </div>
          <div className="chat-toolbar-actions">
            <button className="toolbar-button" type="button" onClick={() => props.onRunContextCommand('/context inspect')} disabled={!snapshot?.permissions.canInspectContext}>
        ...
        <section className="chat-timeline" aria-live="polite">
          {props.turns.length === 0 ? (
            <div className="chat-empty">
              ...
              <p>Pregunta, describe un objetivo o solicita una acción. BAGO mantendrá workspace, sesión y contexto en la misma superficie.</p>
            </div>
        ...
        <section className="chat-composer">
          ...
          <div className="composer-footer">
            ...
            <div className="composer-buttons">
              <button className="icon-button" type="button" onClick={() => props.onRunContextCommand('/context inspect')} disabled={!snapshot?.permissions.canInspectContext} title="Adjuntar contexto">
                <Icon name="attach" />
              </button>
  ...
  if (props.section === 'workspace') {
    const groups = [
      { id: 'sessions', label: 'Sesiones', icon: 'session' as IconName, count: snapshot?.session.id ? 1 : 0, summary: snapshot?.session.id || 'Sin sesión' },
      { id: 'prompts', label: 'Prompts', icon: 'prompt' as IconName, count: 0, summary: 'Gestionados por backend' },
      { id: 'nodes', label: 'Nodos', icon: 'node' as IconName, count: props.routes?.count || 0, summary: `${props.routes?.count || 0} relaciones visibles` },
      { id: 'links', label: 'Enlaces', icon: 'link' as IconName, count: props.routes?.api_prefixes?.length || 0, summary: 'Rutas y bridges' },
      { id: 'artifacts', label: 'Artefactos', icon: 'artifact' as IconName, count: allFiles.length, summary: `${allFiles.length} entradas indexadas` }
    ];
    return (
      <div className="workspace-surface">
        <div className="surface-toolbar">
          ...
          <div className="surface-toolbar-actions">
            <ActionButton icon="plus" primary onClick={() => props.onSetSection('pipeline')}>Nuevo</ActionButton>
            <button className="toolbar-button" type="button" onClick={() => setWorkspaceFilter((current) => current === 'all' ? 'file' : current === 'file' ? 'directory' : 'all')}>
              <Icon name="filter" size={16} /> Filtrar: {workspaceFilter === 'all' ? 'todo' : workspaceFilter === 'file' ? 'archivos' : 'carpetas'}
            </button>
            <button className="toolbar-button" type="button" onClick={() => setWorkspaceLayout((current) => current === 'list' ? 'board' : 'list')}>
              <Icon name="layout" size={16} /> Organizar
            </button>
          </div>
        </div>
        ...
        <div className="workspace-layout">
          <aside className="project-tree">
            <div className="project-root">
              <span className="tree-icon"><Icon name="workspace" size={17} /></span>
              <div><strong>{snapshot?.workspace.id || 'Workspace'}</strong><span>{snapshot?.workspace.repoBranch || snapshot?.workspace.manifestState || 'unknown'}</span></div>
            </div>
            <div className="tree-groups">
              {groups.map((group) => (
                <button
                  key={group.id}
                  type="button"
                  onClick={() => props.onInspect(buildSelection(group.id, 'workspace-group', group.label, group.summary, [`count: ${group.count}`, `workspace: ${snapshot?.workspace.id || 'unknown'}`], group))}
                >
            ...
            <div className="tree-authority">
              <span>Alcance autorizado</span>
              <strong>{snapshot?.workspace.scopeRoot || 'No confirmado'}</strong>
            </div>
          </aside>

          <section className={`workspace-canvas layout-${workspaceLayout}`}>
            <div className="canvas-heading">
              <div>
                <span className="surface-eyebrow">Artefactos</span>
                <h3>Contenido relevante</h3>
              </div>
              <span>{visibleFiles.length} visibles</span>
            </div>
            {visibleFiles.length ? (
                  ...
                  return (
                    <button
                      ...
                      onClick={() => props.onInspect(buildSelection(
                        ...
                        String(entry.name || path || 'Elemento'),
                        path,
                        [`type: ${String(entry.type || 'unknown')}`, `path: ${path}`, `scope: ${snapshot?.workspace.scopeRoot || 'unknown'}`],
                        entry
                      ))}
                    >
                      <span className="artifact-icon"><Icon name={isDirectory ? 'folder' : 'file'} size={17} /></span>
                      <span><strong>{String(entry.name || path || 'Elemento')}</strong><small>{path}</small></span>
              ...
              <div className="empty-state"><Icon name="search" size={24} /><h3>Sin resultados</h3><p>Cambia la búsqueda o el filtro.</p></div>
            )}
          </section>
        </div>
      </div>
      ...
      <div className="graph-surface">
        ...
        <div className="graph-layout">
          ...
          <aside className="recent-nodes">
            <div className="canvas-heading"><div><span className="surface-eyebrow">Recientes</span><h3>Nodos activos</h3></div></div>
      ...
      <div className="pipeline-surface">
        <section className="pipeline-summary">
          ...
          <div className="pipeline-summary-actions">
            ...
            <ActionButton icon="expand" onClick={() => setPipelineExpanded((value) => !value)}>{pipelineExpanded ? 'Contraer flujo' : 'Expandir flujo'}</ActionButton>
          </div>
        </section>
        ...
        <section className="pipeline-timeline">
          <div className="panel-heading"><div><span className="surface-eyebrow">Timeline</span><h3>Pasos de ejecución</h3></div><span>{steps.length} pasos</span></div>
        ...
        {pipelineExpanded && (
          <section className="roadmap-panel">
            <div className="panel-heading"><div><span className="surface-eyebrow">Dependencias</span><h3>Roadmap</h3></div><span>{iterations.length} iteraciones</span></div>
            <div className="roadmap-grid">
              {iterations.map((iteration, index) => (
                <button
                  key={String(iteration.id || index)}
                  type="button"
                  onClick={() => props.onInspect(buildSelection(
                    String(iteration.id || index),
                    'roadmap-iteration',
                    String(iteration.title || `Iteración ${index + 1}`),
                    String(iteration.goal || iteration.status || ''),
                    [`status: ${String(iteration.status || 'unknown')}`, `phases: ${Array.isArray(iteration.phases) ? iteration.phases.length : 0}`, `verification: ${Array.isArray(iteration.verification) ? iteration.verification.length : 0}`],
                    iteration
                  ))}
                >
                  <span>{String(index + 1).padStart(2, '0')}</span>
                  <strong>{String(iteration.title || iteration.id || `Iteración ${index + 1}`)}</strong>
                  <small>{String(iteration.goal || iteration.status || '')}</small>
                </button>
              ))}
            </div>
          </section>
  ...
  if (props.section === 'evidence') {
    const latest = historyMessages.length ? historyMessages[historyMessages.length - 1] : (asRecord(snapshot?.context) || {});
    ...
    const latestSelection = buildSelection(
      ...
      'evidence',
      'Última evidencia',
      summarizeMessage(latest) || `Receipt ${receiptId}`,
      ...
      <div className="evidence-surface">
        <section className="evidence-primary">
          <div className="evidence-heading">
            ...
            <StatusBadge status={snapshot?.context.certificationStatus || snapshot?.context.state || 'unknown'} />
          </div>
          <p className="evidence-narrative">{latestSelection.summary}</p>
          <div className="evidence-facts">
            <Metric label="Estado" value={snapshot?.context.certificationStatus || snapshot?.context.state || 'unknown'} icon="check" />
            <Metric label="Confidence" value={String(latest.confidence ?? latest.score ?? 'No declarada')} icon="context" />
            <Metric label="Source" value={String(latest.source || latest.origin || 'Backend')} icon="server" />
            <Metric label="Receipt" value={receiptId} icon="evidence" />
          </div>
        ...
        {evidenceCompare && previous && (
          <section className="evidence-comparison">
            <div className="panel-heading"><div><span className="surface-eyebrow">Comparación</span><h3>Anterior versus actual</h3></div></div>
        ...
        <section className="evidence-history">
          <div className="panel-heading"><div><span className="surface-eyebrow">Historial breve</span><h3>Evidencias recientes</h3></div><span>{historyMessages.length}</span></div>
          <div className="compact-list">
            {historyMessages.slice(-6).reverse().map((message, index) => (
              <button
                key={`${String(message.id || message.timestamp || index)}`}
                type="button"
                onClick={() => props.onInspect(buildSelection(
                  String(message.id || index),
                  'history-message',
                  String(message.role || 'Evidencia'),
                  summarizeMessage(message).slice(0, 220),
                  [`timestamp: ${String(message.timestamp || message.created_at || 'unknown')}`, `source: ${String(message.source || message.origin || 'history')}`],
                  message
                ))}
              >
                <span className="compact-list-icon"><Icon name="evidence" size={16} /></span>
                <span><strong>{String(message.role || 'evidencia')}</strong><small>{summarizeMessage(message).slice(0, 110) || 'Sin resumen'}</small></span>
                <Icon name="chevron" size={14} />
              </button>
            ))}
          </div>
    ...
    return (
      <div className="context-surface">
        ...
        <section className="context-actions-panel">
          <div className="panel-heading"><div><span className="surface-eyebrow">Acciones</span><h3>Gestionar contexto</h3></div></div>
  ...
  const logAvailable = commandAvailable(props.menu, props.routes, /system logs|\/logs|log/);
  const systemItems = [
    { label: 'Backend', state: snapshot?.system.state || 'unknown', detail: snapshot?.system.healthDetail || (snapshot?.system.backendAvailable ? 'Accesible' : 'No disponible'), icon: 'server' as IconName },
    { label: 'Sesión', state: snapshot?.session.state || 'unknown', detail: snapshot?.session.id || 'Sin sesión', icon: 'session' as IconName },
    { label: 'Workspace', state: snapshot?.workspace.manifestState || 'unknown', detail: snapshot?.workspace.id || snapshot?.workspace.root || 'No confirmado', icon: 'workspace' as IconName },
    { label: 'Modelo', state: snapshot?.model.state || 'unknown', detail: snapshot?.model.effectiveModel || snapshot?.model.configuredModel || 'No confirmado', icon: 'model' as IconName },
  ...
  return (
    <div className="system-surface">
      <section className="system-overview">
        <div className="system-overview-icon"><Icon name={snapshot?.system.backendAvailable ? 'check' : 'warning'} size={26} /></div>
        <div className="system-overview-copy">
          <StatusBadge status={snapshot?.system.state || 'unknown'} />
          <h2>{snapshot?.system.backendAvailable ? 'Sistema operativo' : 'Backend no disponible'}</h2>
          <p>{snapshot?.system.bindingReason || snapshot?.system.healthDetail || 'La UI representa únicamente el estado confirmado por el backend.'}</p>
        </div>
        <div className="system-overview-actions">
          <ActionButton icon="refresh" primary onClick={() => props.onRunCommand('/status')}>Reintentar</ActionButton>
          <ActionButton icon="system" onClick={() => props.onRunCommand('/status')}>Diagnóstico</ActionButton>
          {logAvailable && <ActionButton icon="command" onClick={() => props.onRunCommand('/system logs')}>Abrir log</ActionButton>}
        </div>
      </section>
      ...
      <section className="system-secondary-grid">
        <article className="system-panel">
          <div className="panel-heading"><div><span className="surface-eyebrow">Providers</span><h3>Modelos disponibles</h3></div><span>{providers.length}</span></div>
          <div className="compact-list">
        ...
        <article className="system-panel">
          <div className="panel-heading"><div><span className="surface-eyebrow">Autoridades</span><h3>Rutas confirmadas</h3></div></div>
          <dl className="authority-list">
            ...
            <div><dt>Workspace</dt><dd>{snapshot?.workspace.root || 'No confirmado'}</dd></div>
