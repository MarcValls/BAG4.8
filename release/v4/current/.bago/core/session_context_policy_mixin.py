...
class SessionContextPolicyMixin:
    ...
    def _rag_retrieve(self, user_message: str, limit: int = 3, *, plan: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        include_workspace = include_dynamic or "realtime_tools" in sources_required
        ...
        if include_workspace:
            try:
                workspace_fragments = self._workspace_file_retrieve(user_message, limit=limit)
                fragments.extend(workspace_fragments)
            except Exception:
                pass
