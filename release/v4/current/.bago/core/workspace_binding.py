...
def resolve_framework_root() -> Path:
    ...
    if env_root is not None:
        ...
        if bago_dir.exists():
            return bago_dir
    return Path(__file__).resolve().parents[2] / ".bago"
...
def resolve_workspace_binding(project_root: str | Path | None = None) -> WorkspaceBinding:
    ...
    confirmed = bool(project_exists and workspace_exists and framework_exists and workspace_state_root.name == ".gabo" and Path(scope_root).resolve() == project and not reasons)
