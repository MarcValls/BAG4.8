...
interface Props {
  ...
  onUseInChat: (selection: SelectionRecord) => void;
  onClose: () => void;
}

export function SelectionInspector({ selection, inspectorLevel, onLevelChange, onUseInChat, onClose }: Props) {
  ...
  return (
    <aside className="selection-inspector" aria-label="Inspector de selección">
      ...
      <div className="inspector-actions">
        <button className="secondary-button compact" type="button" onClick={() => void copyText(selection.summary)}>
          Copiar resumen
        </button>
        <button className="secondary-button compact" type="button" onClick={() => void copyText(JSON.stringify(selection.raw, null, 2))}>
          Copiar raw
        </button>
        <button className="primary-button compact" type="button" onClick={() => onUseInChat(selection)}>
          Enviar al chat
        </button>
      </div>
